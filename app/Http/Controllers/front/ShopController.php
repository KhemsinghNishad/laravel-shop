...
class ShopController extends Controller
{
    public function index(Request $request, $categorySlug = NULL, $subcategorySlug = NULL)
    {
        ...
        if (!empty($sortOrder)) {
            if ($sortOrder == 'low') {
                $products = $products->whereBetween('price', [1, 1499])->orderBy('price', 'asc')->with('product_image')->paginate(10);
            } elseif ($sortOrder == 'high') {
                $products = $products->whereBetween('price', [1500, 1000000])->orderBy('id', 'desc')->with('product_image')->paginate(10);
            }
        } else{
